...
class MyNotes {
  ...
  */
  createNote(event) {
    var ourCreatePost = {
      'title': jquery__WEBPACK_IMPORTED_MODULE_0___default()("input.new-note-title").val(),
    ...
    if (ourCreatePost.title.length > 0 || ourCreatePost.content.length > 0) {
      if (ourCreatePost.title.length == 0) ourCreatePost.title = "Default My Note";
      if (ourCreatePost.content.length == 0) ourCreatePost.content = "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.";
      jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
        beforeSend: xhr => {
          xhr.setRequestHeader('X-WP-Nonce', universityData.nonce);
        },
  ...
  editNote(event) {
    ...
    if (thisNote.data("state") == "editable") this.makeNoteReadOnly(thisNote);else this.makeNoteEditable(thisNote);
...
class Search {
  ...
  */
  ...
  keyPressDispatcher(event) {
    // console.log(event.keyCode + " on this a dispatcher test")
    if (event.keyCode == 83 && !this.isOverlayOpen && !jquery__WEBPACK_IMPORTED_MODULE_0___default()("input, textarea").is(':focus')) this.openOverlay();
    if (event.keyCode == 27 && this.isOverlayOpen) this.closeOverlay();
  }
  typingLogic() {
    console.log("typing logic test.");
  ...
  getResults() {
      ...
      this.resultDiv.html(`
                <div class="row">
                    <div class="one-third"><h2 class="search-overlay__section-title">General Information</h2>
                        ...
                        ${results.generalInfo.map(item => `<li>${item.postType == 'page' ? '<b>page:</b>&nbsp;' : ''}<a href="${item.permalink}">${item.title}</a>${item.authorName ? ' by ' + item.authorName : ''}</li>`).join("")}
                    ...
                    <div class="one-third">
                        ...
                        ${results.professors.map(item => `
                            <li class="professor-card__list-item">
                                ...
                                <spam class="professor-card__name">${item.title}</spam>
                                </a>
                            </li>
                        `).join("")}
                    ...
                    <div class="one-third">
                        ...
                        ${results.events.length > 0 ? '<ul class="link-list min-list">' : `<p>No events matches that search. <a href="${universityData.root_url}/event">View all events</a>.</p>`}
    ...
    but delete this code a bit later on
    */
    // $.when(
    ...
    //     this.resultDiv.html(`<h2 class="search-overlay__section-title">General Information ${combinedResults.length ? '(found: '+combinedResults.length+')' : ''}</h2>
    //     ${combinedResults.length ? '<ul class="link-list min-list">' : '<p>No general information matches that search.</p>'}
    //     ${combinedResults.map(item => `<li>${item.type == 'page' ? '<b>page:</b>&nbsp;' : ''}<a href="${item.link}">${item.title.rendered}</a>${item.authorName ? ' by '+item.authorName : ''}</li>`).join("")}
