...
function Quiz(props) {
    const [isCorrent, setIsCorrect] = useState(undefined)
    const [isCorrectDelayed, setIsCorrectDelayed] = useState(undefined)
    useEffect(() => {
        if (isCorrent === false) { setTimeout(() => { setIsCorrect(undefined) }, 2600) }
        if (isCorrent === true) { setTimeout(() => { setIsCorrectDelayed(true) }, 1000) }
    }, [isCorrent])
    function handleAnswer(index) { (index == props.correctAnswer) ? setIsCorrect(true) : setIsCorrect(false) }
    return (
        <div className="paying-attention-frontend" style={{backgroundColor: props.bgColor, textAlign: props.theAlignment}} >
            <p>{props.question}</p>
            <ul>
                {props.answers.map(function(answer, index) {
                    return (<li className={(isCorrectDelayed === true && index == props.correctAnswer ? "no-click" : "") + (isCorrectDelayed === true && index != props.correctAnswer ? "fade-incorrect": "")} onClick={isCorrent == true ? undefined : () => handleAnswer(index)}>
            ...
            <div className={"correct-message" + (isCorrent == true ? " correct-message--visible" : "")}>
            ...
            <div className={"incorrect-message" + (isCorrent === false ? " correct-message--visible" : "")}>
