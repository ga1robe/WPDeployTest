...
/*
  ...
  Version: 1.0
  Author: ga1robe
  Author URI: http://kreatywne-media.pl/graphics_training/
...
class FeaturedProfessor {
    function __construct() {
        ...
        add_action('rest_api_init', array($this, 'profHTML'));
        add_filter('the_content', array($this, 'addRelatedPosts'));
    }
    function onInit() {
        register_meta('post', 'featuredprofessor', array('show_in_rest' => true, 'type' => 'number', 'single' => false), null);
    ...
    function renderCallback($attributes) {
        if(array_key_exists('profId', $attributes) && $attributes['profId']){
    ...
    function profHTML() {
        register_rest_route('featuredProfessor/v1', 'getHTML', array( 'methods' => WP_REST_SERVER::READABLE, 'callback' => [$this, 'getProfHTML'] ));
    }

    function getProfHTML($data) { return generateProfessorHTML($data['profId']); }
